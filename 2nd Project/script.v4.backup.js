...
YOUR TASK:
Improve the request without changing its core intent.

ADD:
1. A clear expert role.
2. Necessary context and assumptions.
3. Specific constraints and quality requirements.
4. A clear output format.
5. A concise success checklist.

OUTPUT:
Return the final optimized prompt first, followed by a short list of improvements made.`;
    $("improveOutput").textContent=result;$("improveOutput").style.display="block";$("improveOutput").dataset.copy=result;
    addHistory("Improved Prompt",result);
  };
}
...
$("generateMain").onclick=()=>{
  const result=buildPrompt($("goalInput").value,$("modelSelect").value,$("toneSelect").value,$("lengthSelect").value,$("langSelect").value);
  $("mainOutput").textContent=result;$("mainScore").textContent=scorePrompt(result)+"/100";
  addHistory("AI Prompt Generator",result);toastMsg("Prompt generated ✨");
};
